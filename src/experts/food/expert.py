...
class FoodExpert(BaseExpert):
    ...
    async def get_response(self, query: str) -> Optional[str]:
        ...
        try:
            ...
            return "I apologize, but I couldn't find a satisfactory answer to your food-related question. Could you please rephrase or provide more details?"
            
        except Exception as e:
            self.logger.error(f"Error in get_response: {str(e)}")
            return "I encountered an error while processing your food-related request. Please try again."
            
    async def _check_local_knowledge(self, query: str) -> Optional[str]:
        """Check local knowledge base for relevant information"""
        try:
            return find_answer(query, get_knowledge_base())
        except Exception as e:
    ...
    async def _generate_ai_response(self, query: str) -> Optional[str]:
        ...
        system_prompt = """You are a food expert assistant. Provide accurate and helpful responses
        about cooking, recipes, ingredients, nutrition, and food culture. If you're not confident
    ...
    async def _perform_web_search(self, query: str) -> Optional[str]:
        ...
        try:
            ...
            search_query = f"{query} yemek tarifi güncel"
            ...
            system_prompt = """Sen bir yemek ve mutfak uzmanısın. Web arama sonuçlarını kullanarak soruya kapsamlı ve doğru bir yanıt üretmelisin.
